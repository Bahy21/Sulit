...
class BuildProductInfo extends StatelessWidget {
  ...
  const BuildProductInfo({Key? key, required this.productDetailsController}) : super(key: key);
  ...
  builder: (context, state) {
    return Wrap(
          children: List.generate(4, (index) => InkWell(
            onTap:()=>productDetailsController.isSelected.onUpdateData(index),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 10,vertical: 7).r,
              margin: const EdgeInsetsDirectional.only(end: 10),
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(5).r,
                border: Border.all(color:state.data==index? context.colors.primary:context.colors.disableGray),
        ...
        BlocBuilder<GenericBloc<int>, GenericState<int>>(
          ...
          builder: (context, state) {
            return Wrap(
              children: List.generate(4, (index) => InkWell(
                onTap:()=>productDetailsController.selectedColorCubit.onUpdateData(index),
                child: Container(
                  width: 40.r,
                  height: 40.r,
